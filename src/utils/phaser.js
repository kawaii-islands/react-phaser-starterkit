...
export const calculateGameSize = (width, height, tileWidth, tileHeight, widthThreshold = 0.5, heightThreshold = 0.5) => {
...
export const getScenesModules = () => {
  ...
  const bootScene = modulePaths.find((modulePath) => getFileNameWithoutExtension(modulePath) === BOOT_SCENE_NAME);
  const otherScenes = modulePaths.filter((modulePath) => getFileNameWithoutExtension(modulePath) !== BOOT_SCENE_NAME);

  return [bootScene, ...otherScenes].map((modulePath) => prepareScene(contextResolver(modulePath), modulePath));
};
